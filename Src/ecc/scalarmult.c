/*
X25519 uNaCl library kernel for checking vectors using basic operations on assembler for cortex M4

Tested on target: STM32F446RE Nucleo Board.
Environment: Keil uVision MDK-ARM 5.18a
Tool chain: V5.06 update 1 (build 61)
Job tester:  Van Gegel, torfone@ukr.net

Exported assembler procedures:
...
Note: old interface fe25519_mpyWith121666  with following fe25519_add
replaced by single fe25519_mpy121666add in two places!
*/
...
)
{
	...
	uint32 initialGuessForNumberOfTimesToSubstractPrime = inout->as_uint32[7] >> 31;
	uint64 accu;
	uint8  ctr;
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!		
//      Replace mpyWith121666 and add with mpy121666add for new interface		
...
)
{
	...
	fe25519_elligator2s_invert(&state.zp, &state.zp, &state.xq, &state.zq, &state.x0, (fe25519*)&state.s);
